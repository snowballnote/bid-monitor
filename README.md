...
- 수행완료 사업은 FMS Drive의 실적증명서·실적증명원 후보를 먼저 조회하고, 없을 때 계약서를 조회합니다.
  ...
  파일명의 사업명·발주처를 정규화하고 사업명 키워드 일치율과 발주처 일치 여부로 추천합니다.
...
`GET /api/drive/list?path=...&company=...`로 지정 폴더와 하위 폴더를 조회합니다.
검색 깊이·폴더 수·파일 수는 `performance.drive.max-depth/max-folders/max-files`로 제한합니다.
설정 누락·인증 실패·조회 오류·검색 제한 초과는 오류로 표시하며 후보 0건으로 취급하지 않습니다.
