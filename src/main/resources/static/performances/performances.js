...
function renderEntry(entry, expanded = false) {
    ...
    const fileCell = el('td');const filename = el('small', entry.selectedFilename || '—');
    filename.className = 'entry-filename';filename.title = entry.selectedFilename || '';fileCell.append(filename);
    ...
    const heading = el('header');heading.className = 'file-panel-heading';
    const collapse = el('button', '접기');collapse.type = 'button';collapse.className = 'entry-collapse';
    heading.append(el('h3', '증빙 파일 관리'), collapse);panel.append(heading);
    const currentSection = el('section');currentSection.className = 'current-file-section';
    const current = el('p', entry.selectedFilename || '지정된 파일 없음');current.className = 'current-evidence-file';
    currentSection.append(el('h4', '현재 지정 파일'), current);
    const columns = el('div');columns.className = 'file-management-columns';
    const fms = el('section');fms.className = 'fms-file-section';
    const candidateHeading = el('header');candidateHeading.className = 'candidate-heading';
    candidateHeading.append(el('h4', 'FMS 후보 파일'));fms.append(candidateHeading);
    ...
    const fileLabel = el('label', 'PC 파일 · 최대 20MB');const fileInput = el('input');fileInput.type='file';fileLabel.append(fileInput);
    const uploadButton=el('button','파일 직접 등록');uploadButton.type='button';uploadButton.className='ui-button ui-button-secondary';
    upload.append(typeLabel,fileLabel,uploadButton);
    const local = el('div');local.className = 'local-file-column';local.append(currentSection, upload);
    columns.append(fms, local);panel.append(columns);
    ...
    const search = el('button', '후보 추천');search.type = 'button';search.className = 'ui-button ui-button-secondary';
    search.title = 'FMS 후보 재조회';candidateHeading.append(search);
    const clear = el('button', '연결 해제');clear.type = 'button';clear.className = 'ui-button ui-button-secondary';
    ...
    actions.append(save, clear);panel.append(actions);detailCell.append(panel);detail.append(detailCell);
    ...
    const toggleDetail = () => {
        ...
        body.classList.toggle('is-expanded', !detail.hidden);
    ...
    collapse.onclick = () => {toggleDetail();toggle.focus();};
