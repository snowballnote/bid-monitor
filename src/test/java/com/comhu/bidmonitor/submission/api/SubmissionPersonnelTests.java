...
class SubmissionPersonnelTests {
    ...
    @BeforeEach void setup() {
        ...
        jdbc.update("INSERT INTO drive_index_root_state(source,company,root,status,folder_count,file_count) VALUES('http://fms.test','CNH','/인력','SUCCESS',1,0)");
    ...
    void index(String filename, String modified) {
        ...
        indexPath(filename, "/인력/개발팀/" + folder + "/" + filename, modified);
    }
    void indexPath(String filename, String path, String modified) {
        jdbc.update("INSERT INTO drive_file_index(source,company,root,name,path,ext,size,last_modified,indexed_at) VALUES('http://fms.test','CNH','/인력',?,?,'pdf',10,?,CURRENT_TIMESTAMP)", filename, path, java.sql.Timestamp.valueOf(modified));
    }
    @Test void personnelSearchUsesFilesRecursivelyIndexedFromSharedBusinessRoot() {
        ...
        try {
            driveProperties.setIndexRoots(List.of("/업무"));
            when(drive.list("/업무")).thenReturn(List.of(new FmsDrivePort.Item("개발팀", "/업무/개발팀", true, 0, null)));
            when(drive.list("/업무/개발팀")).thenReturn(List.of(new FmsDrivePort.Item("01_프로필", "/업무/개발팀/01_프로필", true, 0, null)));
            when(drive.list("/업무/개발팀/01_프로필")).thenReturn(List.of(
                    new FmsDrivePort.Item("프로필_김종수_20260901.pdf", "/업무/개발팀/01_프로필/프로필_김종수_20260901.pdf", false, 10, null)));
            refresh.refresh();
            ...
            assertThat(jdbc.queryForObject("SELECT COUNT(*) FROM drive_file_index WHERE root='/업무'", Integer.class)).isEqualTo(1);
    ...
    @Test void searchUsesOnlyPersonnelFoldersAndDeduplicatesNames() throws Exception {
        ...
        indexPath("프로필_김영수.pdf", "/인력/계약서/프로필_김영수.pdf", "2026-01-01 00:00:00");
        indexPath("프로필_김철수.pdf", "/인력/01_프로필_백업/프로필_김철수.pdf", "2026-01-01 00:00:00");
    ...
    @Test void nameExtractionSkipsAmbiguousAndTemplateNamesAndDepartmentConflicts() {
        ...
        indexPath("김민수_20260901.pdf", "/인력/01_프로필/김민수_20260901.pdf", "2026-09-01 00:00:00");
        ...
        indexPath("김민수.pdf", "/인력/운영팀/03_KOSA경력증명서/김민수.pdf", "2026-09-01 00:00:00");
        assertThat(service.search(caseId, "김민")).containsExactly(new SubmissionPersonnelService.PersonOption("김민수", ""));
    }
    @Test void unrelatedFailedRootDoesNotBlockPersonnelAndWrongFolderCannotSupplyFiles() {
        ...
        indexPath("김대원_20260901.pdf", "/인력/개발팀/01_프로필/김대원_20260901.pdf", "2026-09-01 00:00:00");
        indexPath("프로필_김대원_20260902.pdf", "/인력/개발팀/02_자격사본/프로필_김대원_20260902.pdf", "2026-09-01 00:00:00");
        assertThat(service.search(caseId, "김대")).hasSize(1);
        assertThat(service.candidates(caseId, personId, Type.PROFILE)).hasSize(1).allMatch(file -> file.filename().equals("김대원_20260901.pdf"));
    }
    @Test void remoteZipChecksPermissionAndOnlyReadsSelectedSource() throws Exception {
        ...
        String source = "/인력/개발팀/01_프로필/프로필_김대원_20260911.pdf";
